/* Wildland Ops — service worker.
 * Purpose: let a deployed build announce itself so the crew gets a one-tap
 * "Reload" instead of removing + re-adding the home-screen PWA.
 *
 * SAFETY DESIGN (this app drives readiness decisions):
 *  - This worker ONLY touches same-origin page navigations (the HTML shell).
 *  - EVERY API / cross-origin / live-data request (Firestore, the wildland-auth
 *    Worker, NWS weather, the TFS burn-ban/red-flag proxy, the React CDN) is
 *    left completely alone — straight to the network, NEVER cached. Roster,
 *    currency, and fire-weather data can therefore never be served stale.
 ...
 */
const CACHE = 'wildland-sw-v15';   // <-- BUMP THIS EVERY DEPLOY
